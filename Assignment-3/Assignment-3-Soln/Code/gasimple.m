...
function [bestsol, bestfun, count]=gasimple(dim)
global solnew sol pop popnew fitness fitold f range d;
d = dim;
...
x=range(1):0.1:range(2); plot(x,f(x,dim));
...
function evolve(j)
global solnew popnew fitness fitold pop sol f d;
solnew(j)=bintodec(popnew(j,:));
fitness(j)=f(solnew(j),d);
