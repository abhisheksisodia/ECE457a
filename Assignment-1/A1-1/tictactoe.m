...
beta = intmax
alpha = -1*intmax 
[move] = minimax(board, turn, depth, alpha, beta);
picksquare(handles,move);

function [nextmove, score] = minimax(board, turn, depth, alpha, beta)
availableMoves = find(board==0); 
currentTurn = getappdata(gcbf,'turn');
nextmove = 0;
score = 0;
len = length(availableMoves); 

[win] = checkboard(board);

if win ~= 0 || len == 0
    [score] = checkscore(win, currentTurn, depth)
    return;
end

depth = depth + 1;
scores = []; %arrray of scores
moves = [];  %arrray of moves

for i=1:len % for each possible move
    possibleMove = availableMoves(i);
    moves = [moves, possibleMove]; %push move to moves array
    newBoard = board;
    newBoard(possibleMove) = turn;
    ...
    [~, score] = minimax(newBoard, nextTurn, depth, alpha, beta);
    scores = [scores, score]; %push score to scores array
...
function [score] = checkscore(win, currentTurn, depth)
if win == currentTurn
    score = 10 - depth;
elseif win ~= 0 && win ~= currentTurn
    score = -10 + depth;
else 
    score = 0;
end
